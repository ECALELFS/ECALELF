...
EleSelectionProducers::EleSelectionProducers(const edm::ParameterSet& iConfig):
  ...
  fiducial_selector("fiducial", conversionsHandle, bsHandle, rhoHandle),
  WP70_PU_selector("WP70PU", conversionsHandle, bsHandle, rhoHandle),
  WP80_PU_selector("WP80PU", conversionsHandle, bsHandle, rhoHandle),
  WP90_PU_selector("WP90PU", conversionsHandle, bsHandle, rhoHandle)
...
void
EleSelectionProducers::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for(reco::GsfElectronCollection::const_iterator ele_itr = (electronsHandle)->begin(); 
     ...
     fiducial_selector(*ele_itr, fiducial_ret);
     fiducial_vec.push_back(fiducial_selector.result());

     WP70_PU_selector(*ele_itr, WP70_PU_ret);
     WP80_PU_selector(*ele_itr, WP80_PU_ret);
     WP90_PU_selector(*ele_itr, WP90_PU_ret);
...
#ifdef DEBUG
     std::cout << "[DEBUG] ret=" << WP80_PU_selector.bitMask() << std::endl;
     std::cout << "[DEBUG] ret= (float)" << (SelectionValue_t) WP80_PU_selector.bitMask() << std::endl;
#endif
   }
