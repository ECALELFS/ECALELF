...
#	echo " [INFO] Dataset $dataset"
	...
	if [ -n "${RERECO}" ];then
		...
		./scripts/prodNtuples.sh  -t ${TAGFILE} --type=ALCARERECO \
  			${JSONFILE} ${JSONNAME} \
			--scheduler=$SCHEDULER --file_per_job=10   --doEleIDTree --doExtraStudyTree --doExtraCalibTree --extraName=gainEleFixAll  ${DOEXTRACALIBTREE} ${EXTRAOPTION} ${EXTRAEXTRAOPTION} \
			${TUTORIAL} $dataset 
