...
int main (int argc, char** argv){
  ...
  */
  ...
  try{ EtaBinCenterDeadTT = gConfigParser -> readDoubleListOption("Input::EtaBinCenterDeadTT"); }
  catch(char const* exceptionString ){ EtaBinCenterDeadTT.push_back(-999);}
 
  int NPhiShift ;
  try{ NPhiShift = gConfigParser -> readIntOption("Input::NPhiShift"); }
  catch(char const* exceptionString ){ NPhiShift=14; }

  int PhiOffset ;
  try{ PhiOffset = gConfigParser -> readIntOption("Input::PhiOffset"); }
  catch(char const* exceptionString ){ PhiOffset=4; }

  bool icMapRatio ;
  try{ icMapRatio = gConfigParser -> readBoolOption("Input::icMapRatio"); }
  catch(char const* exceptionString ){ icMapRatio = true;}

  std::vector<double> iEtaBinDivision ;
  try{ iEtaBinDivision = gConfigParser -> readDoubleListOption("Input::iEtaBinDivision"); }
  catch(char const* exceptionString ){ iEtaBinDivision.push_back(0);
  ...
  try{ iPhiBinDivision = gConfigParser -> readDoubleListOption("Input::iPhiBinDivision"); }
  catch(char const* exceptionString ){ iPhiBinDivision.push_back(0);
  ...
  try{ NPhiSMBinDivision = gConfigParser -> readIntOption("Input::NPhiSMBinDivision"); }
  catch(char const* exceptionString ){ NPhiSMBinDivision = 5 ; }
    ...
    if(isDeadTriggerTower){
	  ...
	  if(DeadCrystalEB.back()->GetBinContent(iPhi+1,EtaBinCenterDeadTT.at(iEta)+86)!=0 && EtaBinCenterDeadTT.at(iEta)!=0){
	    ...
	    for( ; iPhiBin < int(iPhiBinDivision.size()-1) ; iPhiBin ++){
             if(iPhi == 0) { iPhiBin = 0 ; break ; }
             else if (iPhi == 360){ iPhiBin = int(iPhiBinDivision.size()-1) -1 ; break;}
  ...
  for( int iPhi = 0 ; iPhi < IPhiWindow ; iPhi ++){
   for( int iEta = 0 ; iEta < IEtaWindow ; iEta ++){
     ...
     for(int iEtaBin = 0 ; iEtaBin < int(iEtaBinDivision.size()-1) ; iEtaBin++){
      ...
      if(!isDeadTriggerTower){GaussianFits_EtaBinned[iEtaBin][iPhi][iEta]->SetLineColor(kBlue);
     ...
     for(int iPhiBin = 0 ; iPhiBin < int(iPhiBinDivision.size()-1) ; iPhiBin++){
      ...
      if(!isDeadTriggerTower){GaussianFits_PhiBinned[iPhiBin][iPhi][iEta]->SetLineColor(kBlue);
     ...
     for(int iSMBin = 0 ; iSMBin < int(20/NPhiSMBinDivision) ; iSMBin++){
      ...
      if(!isDeadTriggerTower){GaussianFits_SMBinned[iSMBin][iPhi][iEta]->SetLineColor(kBlue);
  ...
  for( int iPhi = 0 ; iPhi < IPhiWindow ; iPhi ++){
    for( int iEta = 0 ; iEta < IEtaWindow ; iEta ++){
      ...
      if(!isDeadTriggerTower){ inclusiveGausMeanIC->SetBinContent(iPhi+1,iEta+1,GaussianFits[iPhi][iEta]->GetParameter(1));
      ...
      if(!isDeadTriggerTower){  inclusiveGausRMSIC->SetBinContent(iPhi+1,iEta+1,GaussianFits[iPhi][iEta]->GetParameter(2));
      ...
      if(!isDeadTriggerTower){ inclusiveGausMeanIC_EBP->SetBinContent(iPhi+1,iEta+1,GaussianFits_EBP[iPhi][iEta]->GetParameter(1));
      ...
      if(!isDeadTriggerTower){ inclusiveGausRMSIC_EBP->SetBinContent(iPhi+1,iEta+1,GaussianFits_EBP[iPhi][iEta]->GetParameter(2));
      ...
      if(!isDeadTriggerTower){ inclusiveGausMeanIC_EBM->SetBinContent(iPhi+1,iEta+1,GaussianFits_EBM[iPhi][iEta]->GetParameter(1));
      ...
      if(!isDeadTriggerTower){ inclusiveGausRMSIC_EBM->SetBinContent(iPhi+1,iEta+1,GaussianFits_EBM[iPhi][iEta]->GetParameter(2));
      ...
      for(int iEtaBin = 0 ; iEtaBin < int(iEtaBinDivision.size()-1) ; iEtaBin++){
        ...
        if(!isDeadTriggerTower){ EtaBinned_GausMeanIC[iEtaBin]->SetBinContent(iPhi+1,iEta+1,GaussianFits_EtaBinned[iEtaBin][iPhi][iEta]->GetParameter(1));
      ...
      for(int iPhiBin = 0 ; iPhiBin < int(iPhiBinDivision.size()-1) ; iPhiBin++){
        ...
        if(!isDeadTriggerTower){ PhiBinned_GausMeanIC[iPhiBin]->SetBinContent(iPhi+1,iEta+1,GaussianFits_PhiBinned[iPhiBin][iPhi][iEta]->GetParameter(1));
      ...
      for(int iSMBin = 0 ; iSMBin < int(20/NPhiSMBinDivision) ; iSMBin++){
        ...
        if(!isDeadTriggerTower){ SMBinned_GausMeanIC[iSMBin]->SetBinContent(iPhi+1,iEta+1,GaussianFits_SMBinned[iSMBin][iPhi][iEta]->GetParameter(1));
