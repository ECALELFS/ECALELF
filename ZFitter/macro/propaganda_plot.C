{
	// questo definisce se vengono selezionati elettroni gold
	//#define GOLD
	//  gROOT->ProcessLine(".x macro/load.C");
	//gROOT->ProcessLine(".L ../../../../myTools/ZFit_class/src/ElectronCategory_class.cc+");
	//#include "../../../myTools/ZFit_class/include/ElectronCategory_class.hh"
...
//       data_chain -> Add(selectedTreeDir+"selected-skimZ-rerecoPrompt_V1-RUN2011B"+extension+"-Z_WP80_PU.root");
	...
	TCut EB_cut_ele1 = "abs(etaEle[0]) < 1.4442";
	TCut EB_cut_ele2 = "abs(etaEle[1]) < 1.4442";
	TCut EB_cut = EB_cut_ele1 && EB_cut_ele2 && selection_cut;

	TCut EE_cut_ele1 = "abs(etaEle[0]) > 1.566 && abs(etaEle[0]) < 2.5";
	TCut EE_cut_ele2 = "abs(etaEle[1]) > 1.566 && abs(etaEle[1]) < 2.5";
	TCut EE_cut = EE_cut_ele1 && EE_cut_ele2 && selection_cut;

	TCut gold_ele1_cut = "R9Ele[0] >0.94";
	TCut gold_ele2_cut = "R9Ele[1] >0.94";
...
//   legend.AddEntry(invMass_SC_regrCorr_ele_EB, "E_{SC}");
	...
	data_chain->Draw("invMass_e5x5>>invMass_e5x5_EB(160,50,130)", EB_cut);
	TH1F *invMass_e5x5_EB = (TH1F *) gROOT->FindObject("invMass_e5x5_EB");

	data_chain->Draw("invMass_e5x5>>invMass_e5x5_EE(160,50,130)", EE_cut);
	...
	//  data_chain->Draw("invMass_rawSC_esSC>>invMass_rawSC_EB(160,50,130)", EB_cut);
	...
	//  data_chain->Draw("invMass_SC_regrCorr_ele>>invMass_SC_regrCorr_ele_B_region(160,50,130)", B_region_cut);
	//TH1F *invMass_SC_regrCorr_ele_B_region = (TH1F *) gROOT->FindObject("invMass_SC_regrCorr_ele_B_region");
	...
	legend.Draw();
	pv_EB.Draw();
//   c->SaveAs("propaganda_e5x5_vs_regrCorr_ele-EB"+extension+goldString+".eps");
//   c->SaveAs("propaganda_e5x5_vs_regrCorr_ele-EB"+extension+goldString+".C");
//   c->SaveAs("propaganda_e5x5_vs_regrCorr_ele-EB"+extension+goldString+".png");
	c->SaveAs("E_corrections-EB" + extension + goldString + "-v2.eps");
	c->SaveAs("E_corrections-EB" + extension + goldString + "-v2.C");
	c->SaveAs("E_corrections-EB" + extension + goldString + "-v2.png");
...
//   c->SaveAs("propaganda_e5x5_vs_regrCorr_ele-EE"+extension+goldString+".png");
	...
	c->SaveAs("E_corrections-EE" + extension + goldString + "-v2.eps");
	c->SaveAs("E_corrections-EE" + extension + goldString + "-v2.C");
	c->SaveAs("E_corrections-EE" + extension + goldString + "-v2.png");
}
//------------------------------
