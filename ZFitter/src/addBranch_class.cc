...
TTree* addBranch_class::AddBranch_R9Eleprime(TChain* originalChain, TString treename, bool isMC)
{
	...
	TFile* f = TFile::Open("/eos/project/c/cms-ecal-calibration/data/R9transf/transformation_Moriond17_AfterPreApr_v1.root");
	...
	for(Long64_t ientry = 0; ientry < nentries; ientry++) {
		...
		if(isMC) {
			...
			} else if(abs(etaEle[1]) > 1.566 && abs(etaEle[1]) < 2.5 && R9Ele[0]>0.8) { //endcap
