...
def makeDiagBins(h, nevents=10000):
	xbin = ROOT.Long()
	ybin = ROOT.Long()
	zbin = ROOT.Long()
	...
	while True:
		...
		if hibin > xbins: hbin = xbins
		integral += h.Integral(lowbin, lowbin, startbin, hibin)	
		integral += h.Integral(lowbin, startbin, hibin, hibin)	
		integral -= h.GetBinContent(lowbin, hibin)	
		if integral > nevents or (hibin == xbins and lowbin == 1): break
		i += 1
	...
	while i <= xbins and i <= ybins:
		i += 1
		integral += h.Integral(bins[-1]+1, i, i, i)
	...
	while i > 0:
		...
		if integral > nevents or i == 0:
			bins = [i] + bins
			values = [h.GetXaxis().GetBinLowEdge(i+1)] + values
