{
  ...
  for(std::vector<TString>::const_iterator toySubDir_itr = toySubDirVec.begin();
    ...
    for(int nSmooth=0; nSmooth<=10; nSmooth++){
      if(nSmooth!=0 && nSmooth!=3 && nSmooth!=10) continue;
    ...
    //	nSmooth_itr != intSmoothVec.end(); nSmooth_itr++){
      ...
      TTree *toys = ToyTree(toyDir+toySubDir,"outProfile-scaleStep2smearing_8-Et_25-trigger-noPF-EB.root", optSmooth,nSmooth);
      //toys->Scan("catName:catIndex:constTermTrue:alphaTrue:constTerm:alpha","","col=20");
