...
if ! options=$(getopt -u -o hf:s:d: -l help,runRangesFile:,selection:,invMass_var:,puName:,baseDir:,rereco:,validation,stability,etaScale,systematics,slides,onlyTable,test,commonCut:,period:,noPU,outDirImg:,addBranch:,regionsFile:,corrEleType:,corrEleFile:,fitterOptions: -- "$@")
then
...
do
    case $1 in
		...
		-s) signalNames=($signalNames $2); shift;;
		-d) dataNames=($dataNames $2); shift;;
	--noPU) noPU="--noPU";;
...
do
	...
	if [ -z "$dataName" ];then
		echo "[ERROR] dataName not defined" >> /dev/stderr
		echo ${dataNames[@]}
		exit 1
	fi
	if [ -z "$signalName" ];then
		echo "[ERROR] signalName not defined" >> /dev/stderr
		echo ${signalNames[@]}
		exit 1
	fi
	echo $dataName
...
mv tmp/d_chain.root tmp/$dataName/d_chain.root
mv tmp/s_chain.root tmp/$signalName/s_chain.root
